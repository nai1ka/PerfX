...
Performance strongly shapes how users experience a mobile application and whether they keep using it. Slow response times, high energy use, or a laggy interface frustrate users and often end in poor reviews and uninstalls~\cite{Khalid2015}. Catching such problems early keeps an application usable and its users satisfied, as Tang \emph{et al.}~\cite{Tang} demonstrate. A broad survey of mobile performance optimisation techniques confirms that these problems are pervasive across the field~\cite{HortMobileSurvey}. Two strategies are commonly used to understand application performance: profiling and monitoring. Profiling runs during development or testing and yields detailed, high-resolution data. Monitoring, in contrast, takes place after the application is released and runs on real
...
operating system versions, and user behaviours~\cite{Fragmentation, Crowdsourcing}. This diversity makes it difficult to reproduce rare
...
ADEL, by Zhang \emph{et al.} \cite{ADEL}, targets network performance and energy leaks. It extends the platform to tag downloaded data and follow how that data moves through an application, which reveals unnecessary network activity that wastes energy. The tool is not built for production monitoring, however, because it modifies the Android system and adds high overhead, so it cannot run on real user devices.

At the platform level, Android provides a system tracing infrastructure~\cite{AndroidTracing} used by tools such as Perfetto~\cite{PerfettoDocs}, which record kernel-level events including CPU scheduling, memory pressure, and frame rendering timelines. These tools give detailed visibility into what the system is doing at any moment, but they require a developer-connected device and produce large volumes of data, so they are designed for diagnosing specific issues under controlled conditions rather than for ongoing collection from deployed applications.
...
The main idea of post-release monitoring is to collect metrics from apps as they run on real user devices~\cite{MonitoringAppPerformance}. The main
...
On the user-perceived side, several more specific metrics describe responsiveness. Frame rendering time is the most direct of these. A frame has to be drawn within the display's refresh budget, roughly 16.7 ms on a 60 Hz screen, and a frame that misses this budget shows up as stutter or a dropped frame \cite{AndroidVitals}. Frames per second (FPS) carries the same information in a different form and follows directly from frame time. Startup time, the wait before an application becomes usable after launch, largely sets the user's first impression \cite{AndroidVitals}. % TODO add citation: Android app startup time (vitals/launch-time)
Interaction latency is the delay between a user action and the first visible response. Even when CPU and memory look normal, a long delay here noticeably reduces satisfaction, and human-computer interaction research treats a delay of over one second as the point where the user’s flow of thought is interrupted~\cite{NielsenResponseTimes}. The most severe responsiveness failure is an Application Not Responding (ANR) event, raised when the main thread stays blocked for several seconds, which often makes the user abandon the application~\cite{AndroidVitals, AgingOrGlitching}.
...
Even so, no existing solution combines low-overhead post-release monitoring on Android with cohort-stratified regression detection. This gap is what the proposed system sets out to fill.